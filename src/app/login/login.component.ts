...
export class LoginComponent implements OnInit {
  ...
  ngOnInit() {
    this.currentUser = this.localStorageService.getItemsToLocalStorage();
  ...
  login(user: IUser) {
    ...
    const defaultUser: IUser = { username: 'qwe', password: '123' };
    if (user.username !== '' && user.password !== '') {
      ...
      this.toastService.showToast('danger', 'Login failed! Please specify username and password', 15000);
    }
  }
